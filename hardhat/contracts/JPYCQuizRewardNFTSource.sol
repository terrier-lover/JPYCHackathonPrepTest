...
contract JPYCQuizRewardNFTSource is IJPYCQuizRewardNFTSource, Ownable {
    ...
    function _getSVG(address originalMinter_) private pure returns (string memory) {
        return
            string(
                abi.encodePacked(
                    _getSVGRootOpen(),
                    _getSVGMain1(),
                    _getSVGWalletID(originalMinter_),
                    _getSVGMain3(),
                    _getSVGRootClose()
                )
            );
    ...
    function _getSVGWalletID(address originalMinter_)
        private
        pure
